...
const stopGame = (status) => {
  ...
  document.getElementById("game").innerHTML += `<p>The word was: <span class="result-word">${word}  </span></p><button id="play-again" class="button-primary px-5 py-2 mt-8">Play Again</button>`;
...
export const startGame = () => {
  ...
  gameDiv.innerHTML += `<p id="tries" class="mt-5 mb-5 flex justify-center">TRIES LEFT :  <span id="tries-left" class="font-medium text-red-600 ml-1"> 10 </span></p>`;
  ...
  gameDiv.insertAdjacentHTML('beforeend', `<button id="quit" class="button-secondary px-5 py-1 mt-4">Quit</button>`)
  document.getElementById('quit').onclick = () => {
     const isSure = confirm('Are you sure you quit and lose progress?')
     if (isSure) {
       stopGame('quit')
     }
  }
